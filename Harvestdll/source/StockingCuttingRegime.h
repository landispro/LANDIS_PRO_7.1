#ifndef STOCKINGCUTTINGREGIME_H

#define STOCKINGCUTTINGREGIME_H
...
class StockingCuttingRegime_reentry_event{//add by Qia on May 29 2012
public: 
...
int itsEntryDecade;
	...
	float TargetStocking;//Aug 03 2009

	float Mininum_Stocking; //May 26 2011
...
void StockingCuttingRegime_load_reentry_parameters(FILE *fp);
};

class StockingCuttingRegime : public HarvestRegime {
...
public :
	...
	double GetStockinginACell(int i, int j);
	double GetStockinginACell_spec_age(int i, int j, int spec, int age);
	double computeStandStocking(Stand* stand);
    StockingCuttingRegime();

	//double CutShareStockinginACell(int i, int j,double target);

	double CutShareStockinginACell_LifeSpanPercent(int i, int j,double target);

    virtual ~StockingCuttingRegime();
	...
	StockingCuttingRegime_reentry_event* StockingCuttingRegime_reentry_event_instances;
...
private:
	...
	float TargetStocking_copy;//Aug 03 2009

	float Mininum_Stocking_copy; //May 26 2011
